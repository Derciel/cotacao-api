...
try {
  ...
  // Vamos tentar extrair dados do script JSON que o SwaggerHub coloca na página
  // Geralmente tem algo do tipo window.__reactRouterContext ou window.ENV
  // Ou as definições OpenAPI e textos do corpo
  
  // 1. Encontrar strings JSON ou URLs da API da SIEG
  const urls = content.match(/https?:\/\/[^\s"'`<>]+/g) || [];
  const uniqueUrls = [...new Set(urls)].filter(url => url.includes('sieg'));
  
  // 2. Tentar extrair blocos de texto legíveis do HTML
  ...
  // 3. Procurar por trechos específicos do JSON no SwaggerHub que podem conter as definições de rotas/Swagger
  // Vamos buscar por trechos de Swagger / OpenAPI. Muitas vezes eles vêm codificados.
  const apiInfo = [];
  ...
  output += cleanText.slice(0, 10000) + '...\n'; // primeiras 10k letras
